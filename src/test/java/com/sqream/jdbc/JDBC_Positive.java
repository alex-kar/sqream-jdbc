...
//      log.info();(" - Getting " + table_type + " value back for value");
    ...
    @Test
    public void ACLTest() throws SQLException {
        int INSERT_AMOUNT = 27;
        double expectedRowAmount = 134217728;
        int rowCounter = 0;

        String createTable = "create or replace table public.wmspull_history ( id bigint not null identity(1,1) , joinid bigint not null , transactionid varchar(50) not null , transactiondate date not null , reqreceivedtime datetime not null , enterprisename varchar(100) null , enterpriseid int null , msisdn bigint not null , networktype varchar(10) null , bearer varchar(4) null , circlename varchar(30) null , circleid int null , operatorname varchar(30) null , operatorid int null , destination bigint null , countrycode int null , countryname varchar(56) null , msisdnseries int null , shortcode bigint null , suffix int null , keyword varchar(160) null , receivedmessage nvarchar(4000) null , recdmessagestatus varchar(1) null , recdmessagestatusdesc varchar(200) null , param1 varchar(40) null , param2 varchar(40) null , param3 varchar(40) null , param4 varchar(40) null , param5 varchar(40) null , sender varchar(15) null , responsemessage nvarchar(4000) null , respmessagestatus varchar(1) null , respmessagestatusdesc varchar(200) null , contenttype varchar(2) null , smscid int null , smscname varchar(30) null , messagecount int null , submittime datetime null , responsetime datetime null , total_retries int null , retry1_time datetime null , retry1_errormessage nvarchar(4000) null , retry2_time datetime null , retry2_errormessage nvarchar(4000) null , retry3_time datetime null , retry3_errormessage nvarchar(4000) null );";
        String insert = "insert into wmspull_history values (1, 1, '1', '1955-11-05', '1955-11-05 01:24:00.000', 'enterprisename', 1, 1, 'networktyp', 'bear', 'circlename', 1, " +
                "'operatorname', 1, 1, 1, 'countryname', 1, 1, 1, 'keyword', 'receivedmessage', 'r', 'recdmessagestatusdesc', 'param1', 'param2', 'param3', 'param4', 'param5', 'sender', " +
                "'responsemessage', 'r', 'respmessagestatusdesc', 'co', 1, 'smscname', 1, '1955-11-05 01:24:00.000', '1955-11-05 01:24:00.000', 1, '1955-11-05 01:24:00.000', 'retry1_errormessage', " +
                "'1955-11-05 01:24:00.000', 'retry2_errormessage', '1955-11-05 01:24:00.000', 'retry3_errormessage');";
        String insertSelect = "insert into wmspull_history select * from wmspull_history";
        String select = "select top 50 p.* from(select top 50 REQRECEIVEDTIME,SHORTCODE,MSISDN,keyword,receivedmessage,responsemessage from wmspull_history where reqreceivedtime between '2020-06-06 00:00:00.000' and '2020-06-06 16:59:59.999' and enterpriseid= '3670' order by REQRECEIVEDTIME desc) p;";
        String selectAll = "select * from wmspull_history;";
	    ...
	    try (Connection conn = createConnection();
            ...
            Assert.assertEquals(rowCounter, expectedRowAmount);
        }
    }
}
